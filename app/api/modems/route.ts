...
export async function GET() {
  try {
    ...
    if (error) {
      console.error("GET Modems Error:", error);
      // Fallback to initial modems if table is not yet created or inaccessible
      return NextResponse.json(INITIAL_MODEMS);
    }

    if (!data || data.length === 0) {
      // Auto-seed initial modems
      const { data: seeded } = await supabase
        .from("modems")
        .insert(INITIAL_MODEMS)
        .select();

      return NextResponse.json(seeded || INITIAL_MODEMS);
    }

    return NextResponse.json(data);
  } catch (err) {
    console.error("GET Modems Crash:", err);
    return NextResponse.json(INITIAL_MODEMS);
  }
}
...
export async function PUT(request: Request) {
  try {
    ...
    if (!id) {
      return NextResponse.json({ error: "Modem ID is required" }, { status: 400 });
    }
